...
public interface ITankMovement
{
	...
	/// @attention	isLeft と isRight が共に true の時は転回しませんが、エネルギーは消費します。
	void Move(bool isForward, bool isLeft, bool isRight);
}


/// 戦車の砲塔を制御するためのインターフェース
public interface ITankTurrent
{
	/// 砲塔の転回
	/// 
	/// 砲塔の向きを変えます。@n
	...
	/// @attention	砲塔の転回ではエネルギーを消費しませんが、引数が規定範囲内を超えている場合はペナルティがあります！
	...
	/// @attention	引数が規定範囲内を超えている場合はペナルティがあります！
	void Fire(int force);
}


/// 戦車の更新処理を呼び出すためのインターフェース
public interface ITankUpdater
{
	void Process();
}
